...
internal/
  ...
  osc52/             OSC 52 clipboard policy on the server->client bytes:
                     forwards writes (normalizing an unnamed selection to the
                     system clipboard), drops clipboard read queries by
                     default, --clipboard full|off to widen or close it
...
- **Clipboard is a filter, not an emulator.** `internal/osc52` is the one place gosh does not hand the server's bytes to the terminal verbatim, and both departures are deliberate. A clipboard *read* query (`ESC ] 52 ; c ; ? BEL`) is dropped unless `--clipboard full`: mosh-server relays it (its emulator stores `?` as the clipboard contents and re-emits it in the next diff — verified against mosh 1.4.0), a terminal that answers writes the reply on gosh's stdin, and gosh forwards stdin to the remote — so passing it through hands the far end the local clipboard on request. A write naming no selection (`52;;`) is rewritten to `52;c;` because xterm's default for an empty `Pc` is `s0` (primary plus cut buffer 0), not the clipboard the copying program meant. Everything else, including malformed and non-52 OSC strings, passes through byte for byte; the filter holds only an in-progress sequence and releases it verbatim past a 64 KiB cap. Note what this cannot fix: mosh-server's emulator only recognizes `52;c;`, and tmux's copy-mode emits `52;;`, so a stock tmux copy is destroyed at the server before any client sees it (the reference mosh client fares no better) — the README carries the `terminal-overrides` line that fixes it tmux-side.
