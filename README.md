...
Quit with `Ctrl-^` then `.` (send a literal `Ctrl-^` by pressing it twice); escape processing is disabled when stdin is not a TTY, so piped sessions pass bytes through untouched. The session survives roaming between networks, laptop sleep, and flaky links — that's the point of mosh — including recovering from the stale UDP socket Windows leaves behind after a Wi-Fi/Ethernet switch or sleep/resume.
