...
- **testcontainers integration tests** build a Debian sshd+mosh-server image and are gated behind the `integration` build tag; unit tests must stay Docker-free. The same suite also runs against a rig the caller started, selected by `GOSH_IT_SSH_PORT` (with `GOSH_IT_HOST`, `GOSH_IT_USER`, `GOSH_IT_KEY` or `GOSH_IT_PASSWORD`, and `GOSH_IT_SERVER_COMMAND`) — see `test/integration/rig.go`. That path exists because neither a Linux container nor a GitHub Actions *service* container can run on a macOS or Windows runner, and it is what gives the README's platform table a tested end-to-end row for each OS: macOS uses Homebrew `mosh-server` behind a throwaway sshd, Windows runs `mosh-server` inside WSL 1 + Alpine over 127.0.0.1, while the client under test stays a native Windows binary. Both halves of that are load-bearing and were each arrived at by a red CI run: WSL 1 because it shares the host network stack, where WSL 2 is a NAT'd VM whose localhost relay carries TCP but not the session's UDP, whose Hyper-V firewall drops host-to-VM traffic, and whose mirrored networking mode — the fix for both — is unsupported on the runner and falls back to no networking at all; Alpine because WSL 1's flock() fails on `/etc/passwd`, so any postinst running systemd-sysusers dies, which is how Debian's openssh-server fails to install.
