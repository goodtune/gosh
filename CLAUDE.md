...
- **Receiver rule**: only accept an instruction whose `old_num` is a state we still hold (idempotency enforcement — security-sensitive), dedupe by `new_num`, prune below `throwaway_num`, never regress the ack.
